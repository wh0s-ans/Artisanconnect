...
export default function MyReviews() {
 ...
 useEffect(() => {
   ...
   const loadReviews = async () => {
     try {
       // On cherche les reviews de l'artisan
       const q = query(collection(db, 'reviews'), where('artisanId', '==', user.uid));
 ...
 <span className="text-sm text-editorial-accent font-semibold mb-4 block">Réputation</span>
 <h1 className="text-2xl sm:text-3xl lg:text-5xl font-semibold text-editorial-fg">Mes Avis</h1>
 </div>
 ...
 <div className="text-sm font-semibold opacity-90">{reviews.length} avis vérifiés</div>
 </div>
 ...
 <h3 className="font-semibold mb-6 flex items-center justify-between">Détail des critères</h3>
 <div className="space-y-2 text-xs text-editorial-muted font-medium">
 <div className="flex justify-between items-center"><span className="flex items-center gap-2"><ShieldCheck className="h-3 w-3" />Qualité</span> <span className="text-editorial-fg">{avgRating}/5</span></div>
 <div className="flex justify-between items-center"><span className="flex items-center gap-2"><Clock className="h-3 w-3" />Ponctualité</span> <span className="text-editorial-fg">{avgRating}/5</span></div>
 <div className="flex justify-between items-center"><span className="flex items-center gap-2"><MessageCircle className="h-3 w-3" />Contact</span> <span className="text-editorial-fg">{avgRating}/5</span></div>
 <div className="flex justify-between items-center"><span className="flex items-center gap-2"><ThumbsUp className="h-3 w-3" />Tarif</span> <span className="text-editorial-fg">{avgRating}/5</span></div>
 </div>
 </div>
 </div>

 {loading ? (
   <div className="text-center py-12"><p>Chargement des avis...</p></div>
 ) : reviews.length === 0 ? (
   <div className="bg-secondary/5 border border-dashed border-editorial-border rounded-lg p-12 text-center">
    ...
    <p className="text-editorial-muted font-medium">Aucun avis reçu pour le moment.</p>
   </div>
 ) : (
 ...
 {review.createdAt ? new Date(review.createdAt.seconds * 1000).toLocaleDateString() : 'Avis récent'} • {review.projectTitle || 'Projet'}
 </p>
 </div>
 </div>
 ...
 <span className="text-[10px] uppercase tracking-wider font-bold block mb-2 text-editorial-accent">Votre réponse</span>
...
fs.writeFileSync('src/pages/MyReviews.tsx', c);
