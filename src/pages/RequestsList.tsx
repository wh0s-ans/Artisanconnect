...
const categoryImages: Record<string, string> = {
 ...
 'Peinture': 'https://images.unsplash.com/photo-1581091226825-a6a2a5aee158?auto=format&fit=crop&q=80&w=800',
 'Maçonnerie': 'https://images.unsplash.com/photo-1504307651254-35680f356dfd?auto=format&fit=crop&q=80&w=800',
...
const defaultImage = 'https://images.unsplash.com/photo-1504307651254-35680f356dfd?auto=format&fit=crop&q=80&w=800';
...
export default function RequestsList() {
 ...
 className="max-w-3xl"
 >
 <span className="text-sm text-editorial-accent font-semibold mb-6 block">{t('auto.le-portfolio-ouvert')}</span>
 <h1 className="text-4xl sm:text-6xl lg:text-7xl font-semibold text-editorial-fg leading-none mb-8">
 
                          {t('auto.missions-and')} <br /> <span className="">{t('auto.projets-dexception')}</span>
 </h1>
 <p className="text-editorial-muted text-lg lg:text-xl leading-relaxed ">
 
                          {t('auto.decouvrez-les-appels-doffres-e')}
                          </p>
 </motion.div>
 </div>
 </section>
 ...
 >
 <option value="">{t('auto.tous-les-domaines')}</option>
 {categories.map(c => (
 ...
 <p className="text-editorial-muted text-xl">{t('auto.aucune-opportunite-ne-correspo')}</p>
 </div>
 )}
 ...
 {request.created_at && (new Date().getTime() - new Date(request.created_at).getTime()) < 2 * 60 * 60 * 1000 && (
   <span className="flex items-center gap-1 bg-green-100 text-green-700 text-[10px] font-bold px-2 py-0.5 rounded-sm capitalize border border-green-200 shadow-sm animate-pulse">
     <Sparkles className="h-3 w-3" />  {t('auto.nouveau')}
                                    </span>
 )}
 </div>
 ...
 <div>
 <span className="block text-sm font-bold text-editorial-muted mb-2">{t('auto.enveloppe-budgetaire')}</span>
 <span className="text-xl font-medium text-editorial-fg">
 {request.budget ? `${request.budget} €` : 'Sur devis'}
 </span>
 </div>
 <div>
 <span className="block text-sm font-bold text-editorial-muted mb-2">{t('auto.zone-dintervention')}</span>
 ...
 <div className="inline-flex items-center gap-4 text-sm font-semibold text-editorial-fg group-hover:text-editorial-accent transition-colors">
                          ...
                          {t('auto.candidater')} <ArrowRight className="h-4 w-4 transform group-hover:translate-x-2 transition-transform" />
 </div>
 </div>
 </div>
 </Link>
