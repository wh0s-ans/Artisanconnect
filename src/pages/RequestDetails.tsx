...
export default function RequestDetails() {
 ...
 request.status === 'pending' ? "text-amber-500" : "text-editorial-accent"
 )}>
 • {request.status === 'pending' ? 'Étude de marché' : 'Contrat Assigné'}
 </span>
 ...
 <span className="text-sm text-editorial-accent font-semibold mb-4 block">Catégorie</span>
 <div className="flex items-center gap-3 text-editorial-fg text-sm lowercase ">
 {request.category}
 </div>
 </div>
 <div className="p-6 lg:p-8 bg-editorial-bg">
 <span className="text-sm text-editorial-accent font-semibold mb-4 block">Lieu</span>
 <div className="flex items-center gap-3 text-editorial-fg text-sm ">
 {request.location}
 </div>
 </div>
 <div className="p-6 lg:p-8 bg-editorial-bg">
 <span className="text-sm text-editorial-accent font-semibold mb-4 block">Budget</span>
 <div className="flex items-center gap-3 text-editorial-fg text-sm ">
 {request.budget ? `${request.budget} €` : 'Sur devis'}
 </div>
 </div>
 <div className="p-6 lg:p-8 bg-editorial-bg">
 <span className="text-sm text-editorial-accent font-semibold mb-4 block">Urgence</span>
 <div className="flex items-center gap-3 text-editorial-fg text-sm lowercase ">
 {request.urgency}
 </div>
 </div>
 ...
 <h2 className="text-sm text-editorial-muted font-semibold border-b border-editorial-border pb-4">Descriptif du projet</h2>
 ...
 <h2 className="text-2xl lg:text-3xl font-semibold text-editorial-fg ">Propositions Reçues</h2>
 <span className="text-editorial-muted text-sm font-semibold">Total : {proposals.length}</span>
 </div>
 ...
 <p className="text-editorial-muted text-lg lg:text-xl">En attente des premières offres d'excellence.</p>
 </div>
 ) : (
 ...
 Posté le {new Date(proposal.created_at || Date.now()).toLocaleDateString('fr-FR')}
 </div>
 </div>
 </div>
 <div className="text-left sm:text-right">
 <div className="text-2xl lg:text-3xl font-semibold text-editorial-fg leading-none">{proposal.price} €</div>
 <div className="text-sm font-medium text-editorial-muted font-bold mt-3">Délai : {proposal.estimatedDays} jours</div>
 </div>
 </div>
 <p className="text-editorial-muted text-sm leading-relaxed mb-8 lg:mb-10 ">{proposal.message}</p>
 
 {isOwner && request.status === 'pending' && proposal.status === 'pending' && (
 <button 
 ...
 {isArtisan && request.status === 'pending' && !isOwner && (
 ...
 <h3 className="text-2xl lg:text-3xl font-semibold mb-4">Offre Soumise</h3>
 <p className="text-sm opacity-80 leading-relaxed ">
 Votre étude a été transmise au client avec succès. L'excellence demande de la patience.
 </p>
 </div>
 ) : (
 ...
 <h3 className="text-xl font-semibold text-editorial-fg text-center">Déposer une proposition</h3>
 </div>
 <form onSubmit={handleProposalSubmit} className="p-6 lg:p-10 space-y-8 lg:space-y-10">
 <div className="space-y-4">
 <label className="text-sm text-editorial-accent font-bold">Prix (€)</label>
 <input 
 type="number"
 required
 ...
 <div className="space-y-4">
 <label className="text-sm text-editorial-accent font-bold">Délai (jours)</label>
 <input 
 type="number"
 required
 ...
 className="w-full bg-white border border-editorial-border rounded-md py-2 px-3 text-editorial-fg focus:outline-none focus:border-editorial-accent"
 placeholder="Estimation temporelle"
 />
 </div>
 <div className="space-y-4">
 <label className="text-sm text-editorial-accent font-bold">Message</label>
 <textarea 
 required
 rows={3}
 ...
 <h3 className="text-xl font-semibold text-editorial-fg mb-6 text-center">Êtes-vous artisan ?</h3>
 ...
 <h3 className="text-2xl lg:text-3xl font-semibold text-editorial-fg mb-4 lg:mb-6">Mission Initiée</h3>
 <p className="text-editorial-muted text-sm leading-relaxed mb-8 lg:mb-10 ">La phase opérationnelle a commencé. La communication reste la clé du succès.</p>
 <Link 
 to="/chats"
