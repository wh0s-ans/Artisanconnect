...
export default function PublicProfile() {
 ...
 if (!profile) return <div className="h-screen flex items-center justify-center">{t('auto.profil-introuvable')}</div>;
 ...
 <div className="h-24 w-24 lg:h-40 lg:w-40 mx-auto md:mx-0 border border-editorial-border rounded-lg shadow-sm bg-editorial-bg p-1 lg:p-2 shadow-2xl shrink-0 relative">
  {isOnline && (
    <div className="absolute top-0 right-0 h-4 w-4 bg-green-500 border-2 border-white rounded-full z-10 translate-x-1/2 -translate-y-1/2" title={t('auto.en-ligne')} />
  )}
 ...
 <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
   ...
   <div className="flex items-center justify-center sm:justify-end gap-2">
     <button onClick={handleShare} className="p-2 border border-editorial-border rounded-md text-editorial-muted hover:text-editorial-accent transition-colors relative" title={t('auto.partager')}>
       {copied ? <Check className="h-4 w-4 text-green-500" /> : <Share2 className="h-4 w-4" />}
     </button>
     <button onClick={handleReport} className={`p-2 border border-editorial-border rounded-md transition-colors relative ${reported ? 'text-red-500 bg-red-50 border-red-200' : 'text-editorial-muted hover:text-red-500 hover:bg-red-50 hover:border-red-200'}`} title={t('auto.signaler')}>
 ...
 <p className="text-editorial-muted ">{t('auto.aucune-specialite-renseignee')}</p>
 )}
 </div>
 </section>
 )}
 ...
 <section className="space-y-6 lg:space-y-8">
 <h2 className="text-sm text-editorial-accent font-semibold border-b border-editorial-border pb-4">{t('auto.portfolio-realisations')}</h2>
 ...
 <p className="text-editorial-muted text-sm">{t('auto.archives-photographiques-en-co')}</p>
 </div>
 )}
 </div>
 </section>
 ...
 <h3 className="text-sm text-editorial-accent font-semibold mb-6 lg:mb-10 pb-4 border-b border-editorial-border">{t('auto.details')}</h3>
 ...
 <span className=" text-lg text-editorial-accent ">{t('auto.depuis')} {new Date(profile.created_at || Date.now()).getFullYear()}</span>
 </div>
 </div>
 </div>
