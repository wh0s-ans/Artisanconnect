...
let c = `import React, { useEffect, useState } from 'react';
...
export default function CompareQuotes() {
 ...
 useEffect(() => {
  ...
  const loadQuotes = async () => {
    try {
      ...
      const quotesWithArtisanInfo = await Promise.all(docs.map(async (quote) => {
        if (quote.artisanId) {
          try {
            const userSnap = await getDoc(doc(db, 'users', quote.artisanId));
            if (userSnap.exists()) {
              const u = userSnap.data();
              return { ...quote, artisanName: u.displayName || 'Artisan', rating: u.rating || 0, reviews: u.reviewCount || 0, verified: true };
            }
          } catch(e) {}
        }
 ...
 <Clock className="h-4 w-4 opacity-50" />~ {quote.estimatedDays || '--'} jours
 </div>
 </div>
 ...
 <Link to={\`/artisan/\${quote.artisanId}\`} className="block w-full text-center border border-editorial-border rounded-md shadow-sm py-3 text-sm font-bold text-editorial-fg hover:bg-secondary/5">
...
fs.writeFileSync('src/pages/CompareQuotes.tsx', c);
