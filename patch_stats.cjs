...
export default function Stats() {
 ...
 useEffect(() => {
   ...
   const loadStats = async () => {
     try {
       const q = query(collection(db, 'requests'), where('artisanId', '==', user.uid), where('status', 'in', ['in_progress', 'completed']));
       ...
       docs.forEach((req: any) => {
         ...
         clients.add(req.userId);
         
         if (req.createdAt) {
           const d = new Date(req.createdAt.seconds * 1000);
...
fs.writeFileSync('src/pages/Stats.tsx', c);
